...
The architecture of the analysis pipeline for the diabetic kidney disease example is detailed in Figure \@ref(fig:pipeline). First, we describe how to import data from a tab-delimited spreadsheet into a matrix that is compatible with Numero functions and how to preprocess the data for analysis (Figure \@ref(fig:pipeline)A-D). Next, we create the SOM based on the training set (Figure \@ref(fig:pipeline)B,E). The third segment focuses on map statistics and how to color the maps according to regional variation (Figure \@ref(fig:pipeline)F,G). Lastly, we interpret the results and provide a template on how to manually edit the plots for publications (Figure \@ref(fig:pipeline)H,I).
...
Finally, the  `r Rpackage("Numero")` function that assigns the colors, which we will use in the next section, requires a ratio to indicate the available dynamic range for colors for each variable, which we denote as color amplitudes:
...
The output from *nroCircus()* contains a string of SVG code and the dimensions of the graphical object. To save the plot into an SVG file, you can use the *nroFigure()* command. Figure \@ref(fig:DIABKcoloring) in [Appendix B](#appOthers) shows the resulting coloring.
...
The colorings that were created and saved by the above code snippet are in [Appendix A](#appTraining) for training variables and in [Appendix B](#appOthers) the colorings for all others variables.
...
To make a distinction between diagnostic and other colorings, we used a different color palette for the *nroColorize()* function. The final plot is shown in Figure \@ref(fig:mapQuality)A (post-processed summary figure) in this section and Figure \@ref(fig:Q1coloring) in [Appendix C](#appQuality) (figure created on-the-fly at vignette creation). While there were noticeable differences between the districts, there was a sufficient data point count everywhere on the map and, based on our experience from previous studies, it is unlikely that the results were adversely affected due to sparse representation.

The second quality metric reflects the pattern of missing data on the map. The code template is again the same, with minor modifications. And again, it will not have been executed upon vignette creation as it saves a file.
...
Please note the changes to *nroAggregate()*, title and file name. The results are shown in Figure \@ref(fig:mapQuality)B and in Figure \@ref(fig:Q2coloring) in [Appendix C](#appQuality). The example dataset contained only a few elements with missing values. Therefore, the coverage was close to 100% across the map.

The quality metrics for data point matching helps to identify subsets or individual data points that are not captured by the district profiles. There are two ways to show matching quality, either by coloring the map according to the mean matching errors for districts, or by examining the matching errors of individual data points (also referred to as quantization errors or model residuals). The familiar (non-executed) code template was adapted to produce the coloring:
...
Figure \@ref(fig:biochemistry) shows the map colorings for the training set. Urinary albumin was substantially higher for a subset of individuals located on the top part of the map compared to the lower part (1023 mg/24h versus 10 mg/24h, Figure \@ref(fig:biochemistry)A), and a similar pattern was found for serum creatinine (Figure \@ref(fig:biochemistry)B). Therefore, the individuals with the most advanced stages of chronic kidney disease were located in the upper districts on the SOM. Please note also the dramatic difference in the color intensity between the training set (Figure \@ref(fig:biochemistry)A,B,D-F) and male sex (Figure \@ref(fig:biochemistry)C), which indicates that sex variation was successfully eliminated by the adjustments earlier in the pipeline. Specifically, the observed variation of male-female proportion across the map was within the expected range of random fluctuations (P > 0.05).

The patterns for the lipids were more complicated. Cholesterol showed a pattern of high concentrations in the upper right area and low concentration in the bottom left (6.3 mmol/L versus 4.1 mmol/L, Figure \@ref(fig:biochemistry)D), whereas HDL~2~ cholesterol was the highest in the bottom-right and the lowest in the upper-left (0.73 mmol/L versus 0.37 mmol/L, Figure \@ref(fig:biochemistry)E). Triglycerides showed a general pattern of high concentrations on the upper part of the map (Figure \@ref(fig:biochemistry)F). Of note, the coloring was based on the logarithm of triglyceride concentration, whereas the numbers in the plot indicate absolute concentrations.

We recommend using the SOM together with conventional approaches such as correlation analysis, for broader understanding of the nature of the dataset. For instance, cholesterol and triglycerides were correlated (r = 0.43, P < 0.001), however, the SOM colorings suggest that the correlation may not apply to all individuals; particularly those in the upper-left area with high triglycerides did not seem to follow the linear trend (Figure \@ref(fig:biochemistry)E,F). Other dimension reduction methods such as principal component analysis may work better in datasets where there are clear clusters (a typical SOM analysis may miss the clustering structure) and, again, we recommend using multiple conceptually different methods to achieve robust conclusions. We did not observe any obvious intrinsic clustering structure in the kidney disease dataset (results not shown).
...
The aims of the example study were i) to define and describe metabolic subgroups of type 1 diabetes, and ii) to investigate how the subgroups are associated with mortality. The ability to choose subgroups boundaries on the map while simultaneously observing multiple variables is the main strength of the Numero framework. Furthermore, the intensity of the colorings guides the process towards selecting criteria that have the strongest statistical support. To illustrate the process, we used Figure \@ref(fig:biochemistry) as the starting point to define interesting subgroups and we then compared the subgroups using standard statistical tests. While we admit that our choices for the subgroup boundaries were subjective, we also argue that any observer can dispute those choices and provide an alternative by examining the figures. Therefore, the transparency of the methodology allows collective objectivity that is superior to strict “black box” classifiers, especially when the data patterns overlap and involve multiple outcomes.

The boundaries that capture the main metabolic features of the study cohort are shown in Figure \@ref(fig:subgroups). We started from the region with the highest creatinine (a marker of reduced kidney function), since we expect it to be the most clinically important subgroup with respect to mortality (Figure \@ref(fig:subgroups)A). Next, we split the region with high total or HDL~2~ cholesterol into two (Figure \@ref(fig:subgroups)B,C). Please note that the boundaries may not fit exactly with any specific variable, since we also required that the subgroups have to be mutually exclusive. This is the part where there are no perfect mathematical solutions due to overlaps and multi-morbidity. Lastly, we divided the remaining map area based on the triglyceride concentration (Figure \@ref(fig:subgroups)D). The final subgrouping with district labels is shown in Figure \@ref(fig:subgroups)E.

The second aim of the study was to compare the subgroups with respect to mortality and clinical diagnoses. Graphical comparisons of the metabolic subgroup boundaries and selected map colorings are shown in Figure \@ref(fig:endpoints). Mortality was the highest in the top section of the map (34% in eight years, Figure  \@ref(fig:endpoints)A), and the same region was also characterized by greater than 90% prevalence of diabetic kidney disease (Figure  \@ref(fig:endpoints)B). As expected the High Creatinine Subgroup captured this segment of the study population. In addition, a few districts with increased mortality (up to 22%) and kidney disease prevalence (up to 72%) were found within the High Cholesterol Subgroup, and similar spill-over was observable also in the High Triglyceride Ratio Subgroup. On the other hand, the Low Lipids Subgroup showed the lowest rates of death or complications across all the plots in Figure  \@ref(fig:endpoints).
...
As before, going through all the subgroups and all variables would be tedious, so we prepared a code loop for the subgroup comparisons. The subgroups were copied from Figure \@ref(fig:subgroups)E:
...
# Appendix A: Colorings for training variables {#appTraining}
```{r CHOLcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for serum cholesterol concentration (mmol/L)."}
knitr::include_graphics("CHOL.svg")
```
```{r CREATcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for the logarithm of the serum creatinine concentration (µmol/L)."}
knitr::include_graphics("CREAT_log.svg")
```
```{r HDLcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for the HDL~2~ cholesterol concentration (mmol/L)."}
knitr::include_graphics("HDL2C.svg")
```
```{r ALBcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for the logarithm of the urinary albumin excretion rate (mg/24h)."}
knitr::include_graphics("uALB_log.svg")
```
```{r TGcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for the logarithm of the triglyceride concentration (mmol/L)."}
knitr::include_graphics("TG_log.svg")
```

# Appendix B: Colorings for variables not used in training {#appOthers}
```{r AGEcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for age (years)."}
knitr::include_graphics("AGE.svg")
```
```{r DECEASEDcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for the fraction of deceased individuals at follow-up."}
knitr::include_graphics("DECEASED.svg")
```
```{r DIABKcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for the fraction of individuals with diabetic kidney disease."}
knitr::include_graphics("DIAB_KIDNEY.svg")
```
```{r DIABRcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for the fraction of individuals with diabetic retinopathy."}
knitr::include_graphics("DIAB_RETINO.svg")
```
```{r MACROVASCcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for the fraction of individuals with macrovascular disease."}
knitr::include_graphics("MACROVASC.svg")
```
```{r MENcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for the fraction of men."}
knitr::include_graphics("MALE.svg")
```
```{r METABSYNDRcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for the fraction of individuals with the metabolic syndrome."}
knitr::include_graphics("METAB_SYNDR.svg")
```
```{r T1DDURATcoloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for the duration of type 1 diabetes (years)."}
knitr::include_graphics("T1D_DURAT.svg")
```

# Appendix C: Colorings for map quality {#appQuality}
```{r Q1coloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for data point count."}
knitr::include_graphics("histogram.svg")
```
```{r Q2coloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for data coverage. A coverage of 100% indicates no missing values."}
knitr::include_graphics("coverage.svg")
```
```{r Q3coloring, eval=TRUE, echo=FALSE, fig.width=15, fig.height= 15, fig.cap="Coloring for the mean matching error."}
knitr::include_graphics("quality.svg")
```
