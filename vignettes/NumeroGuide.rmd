...
Please note the changes to *nroAggregate()*, title and file name. The results are shown in Figure \@ref(fig:mapQuality)B. The example dataset contained only a few elements with missing values. Therefore, the coverage was close to 100% across the map.
...
The results are shown in Figure \@ref(fig:mapQuality)C. Again, some regional differences are expected, but there were no indications of serious problems. In particular, the relative quality even in the worst region was close to the average training quality (i.e. close to one).
